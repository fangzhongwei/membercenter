...
/** Entity class storing rows of table TmMemberReg
  ...
  *  @param gmtCreate Database column gmt_create SqlType(TIMESTAMP) */
case class TmMemberRegRow(memberId: Long, regIp: Long, regLat: String, regLng: String, regDeviceType: Byte, regDeviceIdentity: String, gmtCreate: java.sql.Timestamp)
...
trait Tables extends MySQLDBImpl{
  ...
  class TmMemberIdentity(_tableTag: Tag) extends profile.api.Table[TmMemberIdentityRow](_tableTag, "tm_member_identity") {
    ...
    /** Uniqueness Index over (identity,pid) (database name uq_tmi_ip) */
  ...
  implicit def GetResultTmMemberRegRow(implicit e0: GR[Long], e1: GR[Int], e2: GR[String], e3: GR[Byte], e4: GR[java.sql.Timestamp]): GR[TmMemberRegRow] = GR{
    prs => import prs._
      TmMemberRegRow.tupled((<<[Long], <<[Int], <<[String], <<[String], <<[Byte], <<[String], <<[java.sql.Timestamp]))
  }
  ...
  class TmMemberReg(_tableTag: Tag) extends profile.api.Table[TmMemberRegRow](_tableTag, "tm_member_reg") {
    def * = (memberId, regIp, regLat, regLng, regDeviceType, regDeviceIdentity, gmtCreate) <> (TmMemberRegRow.tupled, TmMemberRegRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(memberId), Rep.Some(regIp), Rep.Some(regLat), Rep.Some(regLng), Rep.Some(regDeviceType), Rep.Some(regDeviceIdentity), Rep.Some(gmtCreate)).shaped.<>({r=>import r._; _1.map(_=> TmMemberRegRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column member_id SqlType(BIGINT), PrimaryKey */
    val memberId: Rep[Long] = column[Long]("member_id", O.PrimaryKey)
    /** Database column reg_ip SqlType(INT) */
